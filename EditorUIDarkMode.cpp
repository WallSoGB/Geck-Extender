...
namespace EditorUIDarkMode
{
	...
	const std::unordered_map<char*, ThemeType> TargetWindowThemes
	{
	...
	const std::unordered_set<char*> PermanentWindowSubclasses
	{
		"Creation Kit",
		"ActivatorClass",
		"AlchemyClass",
	...
	LRESULT CALLBACK WindowSubclass(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
	{
		...
		{
			if (HDC hdc = reinterpret_cast<HDC>(wParam))
			{
		...
		case WM_CREATE:
		{
			...
			auto itr = TargetWindowThemes.find(className);
			if (itr != TargetWindowThemes.end())
			...
			if (HTHEME windowTheme = GetWindowTheme(hWnd))
	...
	HRESULT WINAPI Comctl32DrawThemeBackground(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, LPCRECT pRect, LPCRECT pClipRect)
	{
		...
		auto itr = ThemeHandles.find(hTheme);
		if (itr != ThemeHandles.end())
			themeType = itr->second;
